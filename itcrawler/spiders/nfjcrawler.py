		...
		def parse(self, response):
			...
			for i in range(last_button):
				...
				for item in  content_wrapper.xpath(".//a[contains(@class,'posting-list-item')]"):
					yield {
					'title':item.xpath(f"normalize-space({self.FIRST_COL}/h3/text())").get(),
					...
					'keywords':''.join(item.xpath(".//div[@class='posting-info position-relative d-none d-lg-flex flex-grow-1']/nfj-posting-item-tags[@class='ml-3']/nfj-posting-item-tag/object/a/text()").extract()),
